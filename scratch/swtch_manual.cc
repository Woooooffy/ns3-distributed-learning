...
main(int argc, char* argv[])
{
  ...
  LogComponentEnable ("CsmaBridgeExample", LOG_LEVEL_INFO);
    ...
    Ptr<BridgeNetDevice> bridge_dev = DynamicCast<BridgeNetDevice, NetDevice>(bridge.Install(switchNode, switchDevices).Get(0));
		bridge_dev->DisableExpiration();
		// Manual entry programming
		for (uint32_t i = 0; i < switchDevices.GetN(); ++i){
			bridge_dev->AddEntry(Mac48Address::ConvertFrom(terminalDevices.Get(i)->GetAddress()), switchDevices.Get(i));
		}
