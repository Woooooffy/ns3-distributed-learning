...
void SwitchNode::EnableCustomFlowId(bool enable, FlowIdExtractor extractor) {
	m_customFlowId = enable;
	m_flowIdExtractor = extractor;
}

// Called by PointToPointNetDevice::Receive() for switch nodes.
// Packet arrives with PPP already stripped; protocol is e.g. 0x0800 (IPv4).
void SwitchNode::IpForward(Ptr<NetDevice> inDev, Ptr<Packet> packet, uint16_t protocol) {
	if (protocol != 0x0800) return; // only handle IPv4

	// Peek destination IP without modifying the packet.
	Ipv4Header ipHdr;
	{
		Ptr<Packet> copy = packet->Copy();
		copy->PeekHeader(ipHdr);
	}
	uint32_t dip = ipHdr.GetDestination().Get();
	uint32_t sip = ipHdr.GetSource().Get();

	// Optional: custom flow-ID routing (DPI inside UDP payload).
	if (m_customFlowId && m_flowIdExtractor) {
		uint32_t flowId = m_flowIdExtractor(packet->Copy());
		auto flowIt = m_flowIdTable.find(flowId);
		if (flowIt != m_flowIdTable.end()) {
			uint32_t devIdx = flowIt->second;
			if (devIdx < GetNDevices()) {
				Ptr<NetDevice> outDev = GetDevice(devIdx);
				if (outDev != inDev) {
					outDev->Send(packet->Copy(), outDev->GetBroadcast(), protocol);
				}
			}
			return;
		}
		// Fall through to ECMP if no flow-ID rule matched.
	}

	// ECMP routing: look up destination IP in routing table.
	auto it = m_rtTable.find(dip);
	if (it == m_rtTable.end()) {
		NS_LOG_INFO("SwitchNode " << GetId() << ": no route for " << ipHdr.GetDestination() << ", dropping");
		return;
	}
	auto& nexthops = it->second;

	// Hash src+dst IP for ECMP port selection.
	uint8_t buf[8];
	memcpy(buf,     &sip, 4);
	memcpy(buf + 4, &dip, 4);
	uint32_t devIdx = static_cast<uint32_t>(nexthops[EcmpHash(buf, 8, m_ecmpSeed) % nexthops.size()]);
