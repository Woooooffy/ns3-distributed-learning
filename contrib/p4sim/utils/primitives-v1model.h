...
class modify_field_rng_uniform : public ActionPrimitive<Data&, const Data&, const Data&>
{
...
class resubmit : public ActionPrimitive<const Data&>
{
    void operator()(const Data& field_list_id)
    {
        ...
        {
            get_phv().get_field("intrinsic_metadata.resubmit_flag").set(field_list_id);
        }
    }
};
...
class recirculate : public ActionPrimitive<const Data&>
{
    void operator()(const Data& field_list_id)
    {
        ...
        {
            get_phv().get_field("intrinsic_metadata.recirculate_flag").set(field_list_id);
        }
    }
};
...
class modify_field_with_hash_based_offset
    : public ActionPrimitive<Data&, const Data&, const NamedCalculation&, const Data&>
{
    void operator()(Data& dst, const Data& base, const NamedCalculation& hash, const Data& size)
    {
        uint64_t v = (hash.output(get_packet()) % size.get<uint64_t>()) + base.get<uint64_t>();
        dst.set(v);
    }
};
